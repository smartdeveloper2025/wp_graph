...
use The7\Adapters\Elementor\Widget_Templates\General;
use The7\Adapters\Elementor\Widget_Templates\Pagination;
use The7\Adapters\Elementor\Widget_Templates\Sale_Flash;
use The7\Adapters\Elementor\Widget_Templates\Woocommerce\Price;
use WC_Product;
use WP_Query;
	...
	protected function add_title_style_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default'   => '',
				'selectors' => [
					'#page {{WRAPPER}} article:not(.class-1):not(.keep-custom-css) .entry-title a'       => 'color: {{VALUE}}',
					'#page {{WRAPPER}} article:not(.class-1):not(.keep-custom-css) .entry-title a:hover' => 'color: {{VALUE}};',
				],
			]
		);
