...
class The7_Elementor_Simple_Product_Categories_Carousel extends The7_Elementor_Simple_Product_Categories {
	...
	protected function the7_title() {
		return __( 'Simple Product Categories Carousel', 'the7mk2' );
	}
	...
	protected function add_image_style_controls() {
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', '%' ],
				'selectors' => [
					'{{WRAPPER}} .the7-simple-product-thumb img' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
					'{{WRAPPER}} .the7-simple-product-thumb .layzr-bg' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
	...
	protected function add_title_style_controls() {
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .product-content .heading, {{WRAPPER}} .product-content .heading *',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type'      => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .product-content .product-name' => 'color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type'      => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .product-content .product-name:hover' => 'color: {{VALUE}};',
					'{{WRAPPER}} a.post.wrapper:hover .product-content .product-name' => 'color: {{VALUE}};',
				],
			]
		);
				...
				echo $parent_wrapper;
					echo '<div class="post-content-wrapper">';
						if ( $settings['show_product_image'] ) {
							...
							if ( $post_media ) {
								echo '<div class="the7-simple-product-thumb"><div class="the7-product-thumb">' . $post_media .'</div></div>';
							}
						}
	...
	protected function get_category_image( $settings, $category ) {
		...
		if ( $thumbnail_id ) {
			$link_class = [ 'product-thumb', 'img-ratio-wrapper' ];
			if ( ! $link ) {
	...
	protected function less_vars( The7_Elementor_Less_Vars_Decorator_Interface $less_vars ) {
		...
		$less_vars->add_keyword(
			...
			$this->get_unique_class() . '.the7-simple-product-categories-carousel',
			'~"%s"'
		);
